...
export async function depositVault(
  ...
  // Get token accounts
  let vaultTokenAccount = spl.getAssociatedTokenAddressSync(
    mint,
    vaultPDA,
    true
  );
  ...
  if (!tokenAccountInfo || !tokenAccountInfo.data) {
    ...
    ataIx = await spl.createAssociatedTokenAccountInstruction(
      wallet.publicKey,
      vaultTokenAccount,
      vaultPDA,
      mint,
